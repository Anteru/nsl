...
def run(args):
    if args.wasm:
        print(
            "Executing '{}' using '{}'".format(args.MODULE, args.wasm_runtime)
        )
        if args.wasm_runtime == "wasmer":
            ...
            module = wasmer.Module(store, open(args.MODULE, "rb").read())
            ...
            print(
                f"{args.FUNCTION} (" + ", ".join(map(str, invokeArgs)) + ") =",
                f(*invokeArgs),
            )
            ...
            module = wasmtime.Module(
                store.engine, open(args.MODULE, "rb").read()
            )
            ...
            assert isinstance(f, wasmtime.Func)
        ...
        print(
            f"{args.FUNCTION} ("
            + ", ".join(map(str, invokeArgs.values()))
            + ") =",
...
if __name__ == "__main__":
    ...
    runCommand.add_argument(
        "--wasm-runtime", choices=["wasmer", "wasmtime"], default="wasmtime"
    )
