...
def _ConvertType(t: LinearIR.Type) -> WebAssembly.ValueType:
    if t.IsScalar():
        ...
        match type(mt.ElementType):
            case LinearIR.FloatType:
                return _MakeStructForArray(
                    WebAssembly.ValueType.f32, mt.RowCount * mt.ColumnCount
                )
    elif t.IsStructure():
    ...
    if t.IsScalar():
        if isinstance(t, LinearIR.IntegerType):
            return WebAssembly.Instruction(
                WebAssembly.opcodes["i32.const"], (cv.Value,)
            )
        elif isinstance(t, LinearIR.FloatType):
            return WebAssembly.Instruction(
                WebAssembly.opcodes["f32.const"], (cv.Value,)
            )
...
class GenerateWasmVisitor(Visitor.DefaultVisitor):
    ...
    class Context:
        ...
        def OnEnterFunction(self, functionName: str):
            ...
            self.__module.AddExport(
                WebAssembly.Export(self.__functionCount, functionName)
            )
        ...
        if vai.Scope == LinearIR.VariableAccessScope.FUNCTION_ARGUMENT:
            ...
            ctx.Code.AddInstruction(
                WebAssembly.Instruction(
                    WebAssembly.opcodes["local.get"], (index,)
                )
            )
            ctx.Code.AddInstruction(
                WebAssembly.Instruction(
    ...
    def v_BinaryInstruction(self, bi: LinearIR.BinaryInstruction, ctx: Context):
        if isinstance(bi.Type, LinearIR.IntegerType):
            ...
            raise RuntimeError(
                f"Unsupported type for binary operation: {bi.Type}"
            )
        ...
        if operationType == "i32" and bi.OpCode not in {
            LinearIR.OpCode.ADD,
            LinearIR.OpCode.SUB,
            LinearIR.OpCode.MUL,
        }:
    ...
    def v_Function(self, function: LinearIR.Function, ctx: Context):
        ...
        for ref, t in valueReferenceTypes.items():
            valueReferenceToLocalMap[ref] = argCount + c.AddLocal(
                WebAssembly.Local(_ConvertType(t))
            )
...
def GetPass():
    ...
    return nsl.Pass.MakePassFromVisitor(
        GenerateWasmVisitor(ctx), "generate-wasm"
    )
