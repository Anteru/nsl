...
class ComputeTypeVisitor(Visitor.DefaultVisitor):
            ...
            if isinstance(expr, ast.MemberAccessExpression):
                ...
                if parentType.IsPrimitive():
                    if parentType.IsVector() or parentType.IsScalar():
                        ...
                        expr.SetType(
                            ComputeSwizzleType(
                                parentType, expr.GetMember().GetName()
                            )
                        )
                    ...
                    expr.SetType(
                        parentType.GetMembers().GetFieldType(
                ...
                if isinstance(parentType, types.MatrixType):
                    ...
                    arrayType = types.VectorType(
                        parentType.GetComponentType(),
                    ...
                    arrayType = types.ArrayType(
                        parentType.GetComponentType(), nestedSize[1:]
                    )
                    expr.SetType(arrayType)
                else:
    ...
    def v_VariableDeclaration(self, decl, ctx):
        ...
        scope = ctx[-1]
        scope.RegisterVariable(decl.GetName(), decl.ResolveType(scope))
        if decl.HasInitializerExpression():
            self._ProcessExpression(decl.GetInitializerExpression(), scope)
