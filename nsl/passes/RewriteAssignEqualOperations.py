from nsl import ast, op, Visitor


class RewriteAssignEqualVisitor(Visitor.DefaultVisitor):
    ...
    def v_AssignmentExpression(self, node, ctx=None):
        ...
        return ast.AssignmentExpression(
            left, ast.BinaryExpression(operation, left, right)
        )
...
def GetPass():
    ...
    return Pass.MakePassFromVisitor(
        RewriteAssignEqualVisitor(), "rewrite-assign-equal", validator=IsValid
    )
