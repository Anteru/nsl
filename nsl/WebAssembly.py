...
def WriteVec(output: BinaryIO, data: Union[bytes, bytearray]):
...
class Export:
    def __init__(
        self, index: int, name: str, kind: ExternalKind = ExternalKind.Function
    ):
        self.__index = index
        self.__name = name
        self.__kind = kind
    ...
    def AddLocal(self, local: Local):
        ...
        if self.__lastLocal:
            if self.__lastLocal.Type == local.Type:
                self.__lastLocal.SetCount(self.__lastLocal.Count + local.Count)
        else:
